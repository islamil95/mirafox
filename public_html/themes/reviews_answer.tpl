{strip}
	<div class="clearfix">
		<span class="review-avatar kwork-review-avatar">
		...
		<div class="f14 comment">
			<span class="comment-meta"><a href="{$baseurl}/{insert name=get_seo_profile value=a username=$review.answer.worker_username|stripslashes}">{$review.answer.worker_username|stripslashes}</a></span>
			<span class="color-gray db">
				<span class="dib v-align-m f13">{insert name=time_ago assign=cntd value=a days=0 time=$review.answer.time_added}{$cntd} {'назад'|t}</span>
			</span>
			<span class="comment-comment wrap-edit-comment-{$review.RID}">
			...
			{if $review.answer.canEdit}
				...
				<form action='/edit_review_comment'
					  method='post'
					  style='display:none;'
					  class='editForm send-review-js mb20'
					...
					<div class="review-row">
						<div class="js-message-body review-text server-sided"
							 ...
							 placeholder=""
							 name="comment"
							 data-max-count="{\RatingManager::COMMENT_MAX_SYMBOLS_COUNT}">{\Helper::nl2p($review.answer.message)}</div>
