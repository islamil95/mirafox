<div class="order-action-links clearfix" style="background-color: #f6f6f6;">
	{assign var='confirmButtonClass' value=' d-none'}
	{if isAllowToUser($order->USERID) && $order->isAllowedAccept()}
		{assign var='confirmButtonClass' value=''}
	{/if}
	<div class="js-track-form__confirm-block p15-20 pull-left{$confirmButtonClass}">
		<a href="javascript:void(0)" title="{$link}" class="{if $order->has_stages}js-stage-inprogress-approve-link-bottom{else}js-track-form__popup-confirm-link{/if} f14 track-bottom-link-left" data-is-multiple="{if count($order->getReservedStages()) > 1}1{else}0{/if}" data-first-stage="{$order->getReservedStages()->first()->id}">
			{if $order->has_stages}
				{if count($order->getReservedStages()) > 1}
					{'Подтвердить выполнение работы'|t}
				{else}
					{'Подтвердить выполнение задачи'|t}
				{/if}
			{else}
				{'Подтвердить выполнение заказа'|t}
			{/if}
		</a>
	</div>

	{if $allowedCancel && $cancelReasons|@count}
		<div class="p15-20 pull-right mb-20 js-fixed-tooltip">
			{if $order->has_stages && $order->getPaidStages()|count}
				{$cancelText = {'Остановить заказ '|t}}
			{else}
				{$cancelText = {'Отменить заказ'|t}}
			{/if}
			<a href="javascript:void(0)" title="{$cancelText}" class="js-cancel-order-initial-button track-bottom-link-right">
				{$cancelText}
			</a>
			{if $order->has_stages && $order->getPaidStages()|count}
				<span class="v-align-m tooltipster tooltip_circle tooltip_circle--light tooltip_circle--hover" data-tooltip-text="{'Отмена распространяется на задачи, по которым не завершена работа и оплата.'|t}">?</span>
			{/if}
		</div>
		<div class="clear"></div>
		...
		<div class="js-cancel-order-main-popup-template hidden{if isAllowToUser($order->USERID)} isPayer{/if}">
			<form action="{absolute_url route="track_cancel_order"}" method="post">
				...
				<div class="js-inprogress-cancel-default">
					...
					<div class="mt20 order-cancel-form__comment" {if $cancelReasons|@count > 1}style="display:none;"{/if}>
						<label>
							{if isAllowToUser($order->USERID)}
				...
				{if isAllowToUser($order->USERID)}
					<div class="js-inprogress-cancel-reject hidden">
						{if $inprogressCancelRejectCounter == 2}{'Вы уже третий раз пытаетесь отменить этот заказ.'|t}{else}{'Вы уже более трех раз пытаетесь отменить этот заказ.'|t}{/if} {'Возможно, вам требуется помощь третьей стороны, - Арбитра Kwork?'|t}
					</div>
				{/if}

				<div class="popup__buttons mt20">
					<button type="button" class="popup__button js-cancel-order-submit red-btn">
					...
					<button type="reset" class="popup__button pull-right popup-close-js green-btn">{'Пока не отменять'|t}</button>
			...
			<form action="{absolute_url route="track_cancel_order"}" method="post">
				...
				{foreach from=$cancelReasons item=reason key=id}
					{if isset($reason["subtypes"])}
						<input name="parentReason" type="hidden" class="hidden js-cancel-order-parent-reason-input">
						<div data-parent-reason="{$id}" class="js-subreasons">
							{foreach from=$reason["subtypes"] item=subreason key=subid}
								<div>
									<input name="reason" class="styled-radio js-required js-popup-cancel-form__reason js-cancel-order-reason-input" id="reason-{$subid}-placeholder" type="radio" value="{$subid}" data-is-payer="{isAllowToUser($order->worker_id)}"/>
							...
							<div class="mt20 js-cancel-order-comment">
								<label>
									{'Ваш комментарий:'|t}
									<textarea id="message_body4" name="message" rows="5" class="{if isAllowToUser($order->worker_id)}js-required{/if} control-en textarea-styled wMax f14 mt8" style="padding-left:5px; padding-right:5px;"></textarea>
								</label>
							</div>
						</div>
					{/if}
				...
				<hr class="gray mt20" />
				<div class="popup__buttons mt20">
					<button type="submit" class="popup__button js-cancel-order-submit red-btn disabled mt0" disabled>{'Отменить заказ'|t}</button>
					<button type="button" style="height:40px;font-size:16px;display:none;" class="js-cancel-order-back red-btn mt0">{'Отменить по другой причине'|t}</button>
					<button type="reset" class="popup__button popup-close-js green-btn pull-right mt0">{'Вернуться к заказу'|t}</button>
					<div class="clearfix"></div>
				</div>
			</form>
		</div>
	...
	{if isAllowToUser($order->USERID) && $order->isCheck() && $order->data_provided}
		<form action="{route route="track_payer_check_inprogress"}" class="js-track-reject-form hidden" method="post">
