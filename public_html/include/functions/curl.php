...
class Curl_HTTP_Client {
	 ...
	 * @param bool $ssl https or http
	 * @param bool $debug
	 * @access public
	 */
	function __construct(bool $ssl = false, bool $debug = false) {
		$this->ssl = $ssl;
		$this->debug = $debug;
		$this->init();
	}
	...
	function send_multipart_post_data($url, $postdata, $file_field_array = array(), $ip = null, $timeout = 30) {
		...
		if (is_array($postdata)) {
			$post_array = $this->curl_postfields_flatten($postdata);
			$post_string = http_build_query($postdata);
		...
		if (!empty($file_field_array)) {
			foreach ($file_field_array as $var_name => $var_value) {
				...
				if (function_exists('curl_file_create')) { // php 5.5+
					$cFile = curl_file_create($var_value);
				} else { // 
					$cFile = '@' . realpath($var_value);
				}
				$file_field_array[$var_name] = $cFile;
			}
		}
	...
	protected function setHttpHeaders() {
		if (!empty($this->_headers)) {
			...
			curl_setopt($this->ch, CURLOPT_HTTPHEADER, $set);
		}
	}
